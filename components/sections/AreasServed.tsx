...
function Petal({ className = "" }: { className?: string }) {
  return (
    <svg className={`inline-block w-3 h-3 align-middle text-copper-500 ${className}`} viewBox="0 0 24 24" aria-hidden>
    ...
    <section id="areas" className="relative bg-cream py-24 sm:py-32">
      <div className="max-w-[1300px] mx-auto px-5 sm:px-8 grid lg:grid-cols-12 gap-10 lg:gap-16 items-center">
        <div className="lg:col-span-6 order-2 lg:order-1">
          ...
          <div className="mt-10 flex flex-wrap items-center gap-x-2 gap-y-2 max-w-lg">
            {AREAS.map((a, i) => (
              <span key={a} className="inline-flex items-center">
                <span className="font-display italic text-copper-800 text-[clamp(1.25rem,2.2vw,1.85rem)] leading-[1.4] tracking-[-0.005em]">
                  {a}
                </span>
                {i < AREAS.length - 1 && <Petal className="!mx-2" />}
              </span>
            ))}
          </div>
        </div>
